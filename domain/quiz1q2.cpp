...
Quiz1Q2::Quiz1Q2(string name) : State(name)
{
}

spState Quiz1Q2::clone()
{
  return make_shared<Quiz1Q2>(repr());
}

vector<spState> Quiz1Q2::generateChildren()
{
  vector<spState> ret;
  string ch;
  ...
  for_each(ch.begin(), ch.end(), [&](char c) {
      string ss;
      ss = c;
      ret.push_back(make_shared<Quiz1Q2>(ss));
    });
...
bool Quiz1Q2::equal(spState s)
{
  Quiz1Q2* ss = dynamic_cast<Quiz1Q2*>(s.get());
